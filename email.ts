...
export const sendVerificationEmail = async (email: string, code: string) => {
  try {
    const info = await transporter.sendMail({
      from: `"Phỉ Thúy Lê Công" <${process.env.FROM_EMAIL || 'no-reply@jade.example.com'}>`,
      to: email,
      subject: "Phỉ Thúy Lê Công - Mã Xác Nhận Đăng Ký",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #0f5132;">Chào mừng bạn đến với Phỉ Thúy Lê Công</h2>
...
export const sendPasswordResetEmail = async (email: string, link: string) => {
  try {
    const info = await transporter.sendMail({
      from: `"Phỉ Thúy Lê Công" <${process.env.FROM_EMAIL || 'no-reply@jade.example.com'}>`,
      to: email,
      subject: "Phỉ Thúy Lê Công - Khôi Phục Mật Khẩu",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #0f5132;">Yêu cầu Khôi Phục Mật Khẩu</h2>
          <p>Bạn đã yêu cầu đặt lại mật khẩu cho tài khoản Phỉ Thúy Lê Công.</p>
...
export const sendFeedbackRequestEmail = async (email: string, orderId: string) => {
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Phỉ Thúy Lê Công" <${process.env.FROM_EMAIL || 'no-reply@jade.example.com'}>`,
      to: email,
      subject: "Phỉ Thúy Lê Công - Đánh Giá Đơn Hàng Của Bạn",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #0f5132;">Cảm ơn bạn đã mua sắm tại Phỉ Thúy Lê Công!</h2>
...
export const sendWelcomeVoucherEmail = async (email: string, voucherCode: string, discount: number, type: string) => {
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Phỉ Thúy Lê Công" <${process.env.FROM_EMAIL || 'no-reply@jade.example.com'}>`,
      to: email,
      subject: "Phỉ Thúy Lê Công - Quà Tặng Chào Mừng Thành Viên Mới",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: auto; border: 1px solid #eee;">
          <h2 style="color: #0f5132; text-align: center;">Chào Mừng Bạn Đến Với Phỉ Thúy Lê Công!</h2>
          <p>Cảm ơn bạn đã đăng ký trở thành thành viên của gia đình Phỉ Thúy Lê Công. Để chào mừng bạn, chúng tôi xin gửi tặng bạn một mã giảm giá đặc biệt:</p>
