  ...
  return (
    <footer className="bg-teal-900 pt-16 pb-8 border-t border-white/10 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
        <div className="col-span-1 md:col-span-1">
          <div className="flex items-center gap-2 text-white mb-6">
            <h2 className="text-xl font-extrabold tracking-tight uppercase">Phỉ Thúy Lê Công</h2>
          </div>
        ...
        <div>
          <h4 className="font-bold mb-6 text-white uppercase text-sm tracking-widest">{t('footer.links')}</h4>
        ...
        <div>
          <h4 className="font-bold mb-6 text-white uppercase text-sm tracking-widest">{t('footer.support')}</h4>
          <ul className="space-y-4 text-base text-white/70">
        ...
        <div>
          <h4 className="font-bold mb-6 text-white uppercase text-sm tracking-widest">{t('footer.company')}</h4>
          <ul className="space-y-4 text-base text-white/70">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 text-white/50">
        <p className="uppercase text-sm tracking-widest font-bold">{t('footer.rights')}</p>
        <div className="flex gap-8">
          <button onClick={() => setView('privacy-policy')} className="uppercase text-sm tracking-widest font-bold hover:text-white transition-colors">{t('footer.privacy')}</button>
          <button onClick={() => setView('terms-of-service')} className="uppercase text-sm tracking-widest font-bold hover:text-white transition-colors">{t('footer.terms')}</button>
        </div>
        <div className="flex gap-6 items-center">
          {socialSettings?.facebook && (
            <a href={socialSettings.facebook} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors uppercase text-sm tracking-widest font-bold text-white/50 hover:text-white">Facebook</a>
          )}
          {socialSettings?.instagram && (
            <a href={socialSettings.instagram} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors uppercase text-sm tracking-widest font-bold text-white/50 hover:text-white">Instagram</a>
          )}
          {socialSettings?.tiktok && (
            <a href={socialSettings.tiktok} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors uppercase text-sm tracking-widest font-bold text-white/50 hover:text-white">TikTok</a>
          )}
          {socialSettings?.telegram && (
            <a href={socialSettings.telegram} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors uppercase text-sm tracking-widest font-bold text-white/50 hover:text-white">Telegram</a>
          )}
          {!socialSettings && (
            <>
